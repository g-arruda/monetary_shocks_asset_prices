...
3. **`script/clean.R`** — log transforms nominal vars, applies X-13 seasonal adjustment via `R/preprocessing/seasonality.R`. Writes `data/processed/data_log_deseasonalized.csv`. (The `source()` of `R/preprocessing/stationarity.R` is commented out — that file never existed; BLL standardization happens later inside `factor_estimation.R`.)
...
- `script/` — the ordered pipeline plus diagnostics (18 files). `R/` — reusable modules, `source()`d one level deep by `script/`; nothing in `R/` sources anything in `script/`.
