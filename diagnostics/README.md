...
| `04_forca_instrumento.R` | 4 | Documenta a inferência sob instrumento fraco: o que é o "Wald=12", onde cai contra os limiares de bolso, checagens de robustez anteriores, e sinaliza explicitamente que a inversão Anderson-Rubin **fica fora do escopo** desta rodada (adiada, não improvisada). **A lacuna que ele registra em `t4_4_lacuna_AR.csv` foi fechada em 2026-08-10** por `R/identification/weak_iv_ar.R` + `script/ar_bands.R`, com a adaptação que o próprio CSV previa ("identifica nas q inovações e propaga por Λ"); o CSV fica como registro do que se sabia em 07-28. |
