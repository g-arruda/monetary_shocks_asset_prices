...
# `summary()` dele só entra no despacho com o pacote no search path. Sem isso
# o objeto sai sem `@teststat` e o ADF vira NA em silêncio. Não remover.
...
t52 <- data.frame(h = c(6, 12, 24, 36, 48)) |>
  dplyr::mutate(rho_h_OLS = mod[1]^h, rho_h_Kilian = modc[1]^h)
...
cat("\n  resumo:\n")
print(as.data.frame(t54 |> dplyr::count(veredito)), row.names = FALSE)
cat("\n  por tcode:\n")
print(as.data.frame(t54 |> dplyr::count(tcode, veredito) |> dplyr::arrange(tcode)), row.names = FALSE)

nao_est <- t54 |> dplyr::filter(veredito == "I(1) — os dois concordam", tcode == 1)
...
t55 <- data.frame(
  ...
  dplyr::mutate(gap_ols_chi = R2_ols - R2_comum) |>
  dplyr::arrange(dplyr::desc(R2_comum))
...
cat("\n  media por grupo:\n")
print(as.data.frame(t55 |> dplyr::group_by(grupo) |>
        dplyr::summarise(n = dplyr::n(), R2_chi = mean(R2_comum), R2_ols = mean(R2_ols),
                  R2_dif = mean(R2_dif), n_neg = sum(R2_comum < 0),
                  .groups = "drop") |>
        dplyr::arrange(dplyr::desc(R2_chi))), row.names = FALSE, digits = 3)
...
cat("\n  series destacadas:\n")
print(as.data.frame(t55 |> dplyr::filter(var %in% DESTAQUE) |> dplyr::arrange(dplyr::desc(R2_comum)) |>
        dplyr::select(var, grupo, tcode, R2_comum, R2_ols, R2_dif, gap_ols_chi)),
