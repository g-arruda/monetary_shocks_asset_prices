...
| `model_alessi.R` | Script de produção do DFM principal. Consome `production_spec()`, grava a superfície Bai--Ng BLL completa e estima `(r=5, q=5, p=4, choque=+50bp em yield_6m, nboot=800)`. Escreve `output/irf/irf_model_alessi_r5q5.pdf`. |
...
| `fig_section5.R` | Pós-processamento puro: lê o `irf_coherence_cell.rds` canônico e as tabelas da Tarefa 7, sem reestimar nada. Escreve as 10 figuras `paper/fig_*.pdf` até h=36; as figuras fiscais e de expectativas usam exclusivamente o cache de produção de 115 séries. |
