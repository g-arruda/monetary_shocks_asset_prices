...
| script | o que faz |
|---|---|
| `instrument_diagnostics.R` | Compara as variantes em um DFM compartilhado (r=8,q=8,p=6), reportando ξ_mp e o primeiro estágio robusto F_rob,mp na direção de `yield_6m`, além da dispersão Copom-dia e do teste de variância Copom vs. não-Copom. Escreve `output/instrument/instrument_diagnostics_report.md` + um PNG. |
...
- **`rm(list=ls())`**: presente em 20 dos 26 arquivos. Ausente em `download.R`, `instrument.R`, `instrument_diagnostics.R`, `run_all.R`, `validate_hac_kernel.R` e `validate_olea_kilian.R` — os seis são pensados para rodar em processo `Rscript` próprio, não para ser `source()`ados numa sessão existente.
