...
**Recomendação após auditoria + validação (2026-05-05/06):** `z_het_jk_3var` com normalização em `yield_6m`. F de primeiro estágio (y6m AR) = 55.98 (vs 21.29 do `z_het_jk` 4-var, vs 1.1 para `z_het` em `juros_selic`). T5 anti-JK F = 0.19 e T6 F-curve confirmam que o filtro JK é informativo, não só esparsificação. T7 (AR-order p ∈ {3,6,12}) confirma F estável (20.7 / 21.3 / 22.4). T8 (Andrews QLR sup-F = 6.88) **fail to reject** evidência formal de quebra estrutural no slope do first-stage — o drop F sub-period é mecânico (var(innov) cresce 3.6× pós-COVID), não regime change. A3 het-ID separado pre/post-COVID: cosine(b_1) = 1.000, norm_ratio = 0.687 — direção do impact column estável; "A3 sustained". **IRF cross-instrument (2026-05-06 tarde):** `z_het_jk_3var` recupera o canal de desinflação de GRG (Δipca = -0.10 pp ≈ GRG Δπᵉ 1y -0.13 pp); `z_jk_purif` falha (Δipca ≈ 0) — evidência discriminante. Sinal oposto vs GRG em BRL/USD (+2.17% vs -2.55%) e CDS interpretado como fiscal-dominance dynamics capturadas pela agregação mensal do DFM (vs IV diário de GRG). Ver `_instrucoes/Heteroscedasticidade.md`, `output/instrument_audit_report.md`, `output/instrument_diagnostics_report.md`, `output/het_validation_report.md`, `output/irf_section.md`.

Para a seção empírica do paper (§5), os artefatos prontos são `output/irf_section.md` (texto), `output/irf_{zhetjk3var,zjkpurif,comparison}.pdf` (figuras) e `output/grg_benchmark.csv` (tabela numérica per +50bp).
