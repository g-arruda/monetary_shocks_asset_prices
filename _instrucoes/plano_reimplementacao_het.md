...
- **B — Sistema diário RS-2004 como headline + ponte explícita para o DFM.** Camadas 1-3 atuais (sem filtro JK) viram a identificação declarada, com resultados diários à la RS-2004 (impacto sobre curva/IBOV/BRL); o DFM mensal consome o choque agregado via projeção — a mecânica proxy permanece, mas o texto a assume explicitamente como ponte, com as hipóteses de agregação declaradas.
...
**Desfecho empírico (2026-07-16, fase de código):** a arquitetura A foi **reprovada pelos gates em todo o grid** (16 células): o placebo de permutação de labels não distingue o calendário Copom (p_perm 0.26–0.86; única marginal pre_covid (8,8) p=6 = 0.037) e o LR de proporcionalidade nunca rejeita Σ_C ∝ Σ_NC — A1/rank condition não existem na frequência mensal (a agregação dilui a elevação de variância de dia de anúncio; spanning do `yield_6m` é bom, 0.70–0.97, o problema é a frequência, não o span). Relatório: `output/het_primary/feasibility_report.md`. Pela regra de parada (decisão do autor), a produção segue proxy `z_jk_bs_purif`; o código do ramo het está implementado e validado (`R/identification/het_primary.R`, `identification = "het"` em `compute_irf_dfm`/`main_sdfm`, harness `script/validate_het_primary_sim.R` 100%). **A decisão entre o fallback B (sistema diário RS-2004 declarado + ponte proxy) e manter o desenho proxy atual volta ao autor.**
