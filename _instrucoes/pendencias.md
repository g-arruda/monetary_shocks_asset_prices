...
- [x] **A-episódio (BPSS 2021) testada** — *reprovada 2026-07-16*
  `script/het_episode_feasibility.R` (S2 pré/pós-2020 + S4 fina, 4 células (r,q)): proporcionalidade nunca rejeitada (volatilidade = escala comum), autovalores generalizados indistinguíveis (min gap 0.04-0.19), coluna monetária com λ ≈ 1. Het mensal morta nas duas variantes (calendário e episódio). `output/het_primary/episode_feasibility_report.md`.
- [x] **Decisão do autor (2026-07-16): abandonar QUALQUER instrumento/proxy** — fallback B removida do plano; material diário RS-2004 só como evidência descritiva, nunca identificação.
- [ ] **DECISÃO DO AUTOR pendente: nova identificação primária na frequência mensal.** Menu sem instrumentos e sem het (ambas reprovadas empiricamente): sign restrictions set-ID (frequentista Gafarov-Meier-Montiel Olea ou bayesiana Uhlig/Arias-Rubio-Ramírez-Waggoner, combinável com o SDFM), recursiva/Cholesky, não-gaussianidade/ICA (Lanne-Meitz-Saikkonen 2017 — diagnóstico barato: testes de normalidade sobre η), GARCH-het (T=147, pouco provável). Sweep/validação/§5/tex condicionados a essa escolha.
