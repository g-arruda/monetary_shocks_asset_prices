...
> **A máscara JK não seleciona risco soberano para dentro; ela seleciona *menos* risco que um dia comum.** Item de topo do council review de 07-31, testado em `script/jk_sovereign_confound.R` → `output/instrument/jk_sovereign_confound.{csv,md}` + `jk_sovereign_days.csv` + `jk_sovereign_irf_overlay.pdf`; leitura em `relatorio/working-notes/2026-07-31_confound_soberano_jk.md`. **Nada aqui mudou** — `DEFAULT_VARIANT`, vértice, esquema de agregação e a cadeia de `build_variants.R` seguem intocados.
...
> 2. **O dado aponta ao contrário.** Regra de leitura fixada *antes* dos números: contaminação exige que o dia retido carregue **mais** risco por unidade de surpresa que um dia comum. Nas 498 quintas **não-Copom** ΔEMBI carrega a surpresa com coef **0,326** (t = 3,97, R² 0,13); nos **62 dias retidos**, **0,099** (t = 1,74, R² 0,04). No câmbio, 0,051 (t = 4,64) contra 0,004 (t = 0,40). As interações `x:1(jk_bs)` são **negativas** nas quatro proxies da janela do evento — EMBI −0,182 (p_boot 0,108), **BRL −0,036 (p_boot 0,066**, apreciação relativa = assinatura de UIP), slope DI −0,228, DI 10a −0,616. A camada BS + máscara **empobrece** o conteúdo de risco.
> 3. **⚠ Pré-requisito que quase inverteu o veredito: o alinhamento do EMBI.** O arquivo (`data/banco_central_rep_dominicana/embi_brasil.csv`) é painel JP Morgan republicado pelo BC dominicano. Correlação de ΔEMBI com o mercado em t / t−1: S&P −0,498 / −0,045; VIX +0,413 / −0,007; Ibov −0,508 / −0,088. **É do mesmo dia.** Logo a janela Qui→Sex (interação **+0,248, p_boot 0,025**) **não** é correção de defasagem — é a resposta *defasada* do risco, que a IRF mensal já reporta. Não re-derivar isso.
> 4. **Classificação de três vias** (política / soberano / informação), terceira via pelo sinal do câmbio purificado na mesma RHS pré-evento do BS: os 62 partem em **31 / 30 / 1** (regra FX; 24/37/1 pela do EMBI) e **nenhum sinal inverte**. ξ_mp cai a ~3,5 nas duas metades — queda mecânica, os meses não-nulos vão de 62 para ~30. **Não promover à produção:** custa metade da amostra e não muda sinal nenhum.
> 5. **O achado mais forte.** Os 31 dias "política" foram selecionados por terem **apreciação do BRL no dia do evento**, e a IRF mensal deles ainda dá **depreciação** (+0,129 contra 0,150 da produção). A depreciação do §4 é propagação mensal, **não** seleção de dias — e o desenho tornava isso falsificável.
> 6. **Ortogonalizar `e_di_bs` ao risco diário** (ΔEMBI + Δlog BRL, R² 0,127) é um **limite inferior**, pois política move spread legitimamente: ξ_mp **10,72** full (acima dos 10,43), 8,18 pré-COVID, todas as manchetes sig90. Sobreviveu.
> 7. **⚠ A ressalva que fica e vai ao paper:** concentração alta — os 5 dias de maior alavancagem valem **28,6%** de Σ|z|, e o maior (6,6%) é **2021-10-27, a semana da PEC dos Precatórios**, com o BRL depreciando 1,97% apesar de alta de 150 pb. O teste agregado não detecta enriquecimento sistemático, mas o dia individualmente mais influente é exatamente o tipo temido.
> 8. **Lacuna declarada:** não há **CDS 5a diário** no repo nem fonte programática gratuita com histórico 2013-2025 (Ipeadata encerrou o EMBI+ em 07/2024 e nunca teve CDS; WorldGovernmentBonds sem CSV/API; MacroMicro é semanal; cbonds é pago). Única fonte diária: página histórica da Investing.com, export por navegador. O script detecta `data/investing/cds5y_daily.csv` sozinho se o arquivo aparecer.
