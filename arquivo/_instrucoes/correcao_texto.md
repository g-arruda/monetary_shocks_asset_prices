...
- **Normalização:** choque que eleva `yield_6m` em 50 bps (controlado por `mp_var` em `main_sdfm`; ver `registro/justificativa_uso_yield-6m.md`).
...
- **Variável de política recomendada para o proxy-SVAR:** `yield_6m` (passa Stock-Yogo F (y6m AR) > 10 em todas as variantes JK; passa Stock-Yogo F (factor-space) ≥ 10 apenas em `z_jk_purif`). `juros_selic` apresenta atenuação severa por descasamento de maturidade com o anúncio Copom. Justificativa em `registro/justificativa_uso_yield-6m.md`.
- **Dados brutos mensais:** `data/raw/raw_data.csv`.
...
- **Painel de instrumentos lado-a-lado:** `data/processed/instrumentos_mensais.csv` (6 GK + 4 het = 10 colunas; as duas GK adicionais de 2026-07-14 são o par de máscara bruta `z_jk_raw_purif`/`z_jk_raw_purif_local` — ver `notas/2026-07-14_ordem_purificacao_jk.md`).
- **Dados diários para identificação por heterocedasticidade:** `data/raw/di.csv`, `data/raw/yields/yields_dia.csv` (Svensson), `data/processed/ibov_daily.csv`, `data/processed/brl_usd_daily.csv`, `data/raw/copom_historico.csv`.
- **Curva de juros (Svensson):** `data/raw/yields/yields.csv` (mensal), `data/raw/yields/yields_dia.csv` (diária), `data/raw/yields/fatores.csv`.
