...
A ortogonalização por informação opera no dia do anúncio e não garante que o instrumento, uma vez agregado em frequência mensal, seja imprevisível. Testamos essa previsibilidade regredindo o instrumento mensal sobre defasagens dos fatores estimados e dos retornos das séries externas\footnote{As séries externas usadas nas cinco especificações são o retorno do S\&P 500 combinado à variação do VIX, o retorno do MSCI de mercados emergentes, a variação do índice de commodities metálicas do Banco Central e a variação do índice de incerteza de política econômica dos Estados Unidos, e o câmbio USD/BRL entra como quinto regressor, de natureza doméstica.}, em cinco especificações\footnote{As cinco especificações são o teste conjunto dos retornos das séries externas com três e com seis defasagens mensais, e o teste conjunto dos fatores estimados do modelo com uma, três e seis defasagens mensais.}, com estatística de Wald robusta a heterocedasticidade. Como o instrumento vale zero em 94 dos 156 meses do calendário, o valor-$p$ assintótico não é confiável, e os valores reportados vêm de \textit{wild bootstrap} sob a nula com 2.000 réplicas. Nenhuma das cinco especificações rejeita a imprevisibilidade, com $R^2$ entre $0{,}054$ e $0{,}395$ e valores-$p$ de \textit{bootstrap} entre $0{,}199$ e $0{,}854$.

A única suspeita de exogeneidade que a Seção 4 levanta vem dos dois últimos painéis da \autoref{fig:cambio}. O índice de commodities metálicas do Banco Central sobe $3{,}43\%$ no impacto e é significativo a 90\% até $h = 4$, embora uma economia pequena e aberta não mova o preço mundial dos metais. A resposta seria sinal de que o instrumento retém um componente global de commodities ou de risco, hipótese que a denominação do índice permite testar diretamente.

O índice do Banco Central é medido em reais, e comparar cada série com sua versão em dólares dentro de uma mesma estimação separa as duas hipóteses. Em um painel aumentado com os três índices de commodities convertidos para dólares, estimado com 200 réplicas, o índice de metais em reais responde $+3{,}98\%$ no impacto e é significativo em 4 dos 5 primeiros horizontes, enquanto o mesmo índice em dólares responde $+0{,}59\%$ e não separa de zero em nenhum dos 25 horizontes testados. Os índices de agropecuária e de energia se comportam do mesmo modo, e a correlação entre a variação do índice de metais e a do câmbio é de $+0{,}450$ na denominação em reais contra $-0{,}409$ na denominação em dólares.

Um fator global moveria as duas denominações, e a versão em dólares não se move em horizonte algum, de modo que o movimento do índice em reais é a resposta cambial herdada por um preço doméstico. A magnitude confirma a leitura, uma vez que os $+3{,}98\%$ do índice em reais são praticamente os $+3{,}27\%$ do câmbio estimados no mesmo painel. A comparação é interna ao painel aumentado, cuja $\xi_{mp}$ cai para $7{,}87$ com a inclusão das três séries, o que a torna válida como contraste entre denominações e não como estimativa alternativa da magnitude de produção.

As três séries externas usadas como placebo passam nos dois níveis de leitura adotados na Seção 4. Na banda de 90\%, o S\&P 500 combinado ao VIX exclui zero em 1 dos 49 horizontes, o MSCI de mercados emergentes em nenhum e o índice de incerteza de política econômica dos Estados Unidos em nenhum, contra as cerca de 5 rejeições por série que um nível nominal de 10\% produziria por acaso. Na banda de 68\%, que sustenta a leitura de médio prazo, são 3 horizontes no S\&P 500, nenhum no MSCI e 2 no índice de incerteza, somando 5 dos 147 pares contra os cerca de 47 que um nível nominal de 32\% produziria.

\begin{figure}[H]
\begin{center}
    \includegraphics[scale=0.7]{fig_placebos.pdf}
\caption{\textbf{Placebos}\\{Séries externas que um choque de política monetária brasileiro não deve mover. O único marcador laranja está em $h = 2$ no primeiro painel, 1 rejeição em 49 horizontes contra as cerca de 5 que um nível nominal de 10\% produziria por acaso. A faixa escura de 68\% exclui zero em 5 dos 147 pares das três séries, contra as cerca de 47 que um nível nominal de 32\% produziria.}}
\label{fig:placebos}
\end{center}
\end{figure}
...
Ortogonalizar a surpresa ao risco soberano contemporâneo é o limite inferior do exercício, porque política monetária move o spread soberano legitimamente e a operação remove também essa parte. O instrumento assim construído fica mais forte, e não mais fraco, com $\xi_{mp}$ passando de $10{,}43$ na produção para $10{,}72$ quando a regressão auxiliar usa EMBI+ e câmbio e para $12{,}68$ quando o CDS entra nela. Nessa última versão as respostas de manchete permanecem significativas a 90\% e o câmbio praticamente não se move, com $0{,}139$ contra $0{,}150$ da produção nas unidades do painel, valor este que corresponde à depreciação de $3{,}64\%$ da Seção 4.2, e o CDS em $26{,}3$ contra $29{,}1$ pontos-base. Se a máscara estivesse selecionando risco soberano, retirar o risco contemporâneo destruiria o primeiro estágio.

Essa operação limpa os valores da surpresa sem tocar na seleção dos dias, que é justamente o que o filtro de sinal faz, e a distinção importa porque a força do instrumento vem da máscara e não dos valores purificados. Como a regra de sinal compara a surpresa de juros com o retorno acionário do mesmo dia, um segundo exercício ortogonaliza também o retorno do Ibovespa ao mesmo bloco de risco e re-deriva a regra sobre os dois resíduos. É pela perna acionária que a seleção pode mudar, uma vez que o bloco de risco explica $15{,}4\%$ da variância da surpresa de juros mas $40{,}4\%$ da do retorno acionário, e ela de fato muda, porque a máscara re-derivada retém 63 dias dos quais apenas 50 estavam entre os 62 originais.

O primeiro estágio dessa partição é mais fraco, com $\xi_{mp}$ de $5{,}57$ na amostra cheia, o que mantém o conjunto de Anderson-Rubin limitado mas fica abaixo do patamar em que as bandas convencionais se sustentam, de modo que a variante autoriza leitura de sinal e não de intervalo. Nenhuma resposta significativa inverte, e as da curva, do câmbio e do risco soberano ganham amplitude, com o câmbio em $0{,}248$ e o CDS em $63{,}9$ pontos-base, embora parte dessa amplificação seja aritmética, porque o denominador de normalização cai a $0{,}726$ do valor de produção. A única inversão está no IPCA no impacto, que passa de $-0{,}070$ a $+0{,}082$ sem ser significativo em nenhuma das duas versões, e o Ibovespa no impacto passa de nulo a negativo e significativo a 90\%, ganho que a fraqueza do primeiro estágio dessa variante não permite carregar para a Seção 4.6. Na janela anterior à pandemia a ordem se inverte e a máscara re-derivada entrega $10{,}94$ contra $12{,}22$ da produção, o que localiza a perda de força no período em que juros e risco soberano se moveram juntos. A leitura defensável é a de que o filtro reduz o conteúdo de risco soberano dos dias que retém sem eliminá-lo, e a de que o resultado central sobrevive a remover esse conteúdo tanto dos valores da surpresa quanto da regra que seleciona os dias.
