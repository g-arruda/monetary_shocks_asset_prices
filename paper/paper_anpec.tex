...
A construção do VAR dos fatores segue \cite{lenzaprimiceri} porque as inovações fatoriais de março e abril de 2020 têm magnitude incompatível com o resto da amostra, e mínimos quadrados ordinários as leem como variação regular do processo, o que desloca os coeficientes autorregressivos e a covariância das inovações que a identificação projeta sobre o instrumento. O escalar $s_t$ em \ref{eq:5} absorve essa magnitude em uma reescala comum, estimada junto do VAR, e mantém os meses da pandemia na amostra.\footnote{\citeauthor{lenzaprimiceri} escrevem $y_t = C + B_1 y_{t-1} + \dots + B_p y_{t-p} + s_t \varepsilon_t$, com $y_t$ o vetor $n \times 1$ de observáveis. Aqui $y_t$ é substituído por $F_t$, o vetor $r \times 1$ de fatores estáticos estimados, e $n$ por $r$. A trajetória de $s_t$ e a verossimilhança que a estima são as dos autores, sem alteração de forma.}

A reescala vale $s_t = 1$ antes de $t^*$, o primeiro mês de variação anômala, e
\begin{equation}\label{eq:5b}
    s_{t^*} = \bar{s}_0, \qquad s_{t^*+1} = \bar{s}_1, \qquad s_{t^*+2} = \bar{s}_2, \qquad s_{t^*+j} = 1 + (\bar{s}_2 - 1)\rho^{\,j-2},
\end{equation}
de forma que os três primeiros meses recebem escalas livres e os seguintes retornam a um à taxa geométrica $\rho$. O vetor $\theta \equiv [\bar{s}_0, \bar{s}_1, \bar{s}_2, \rho]$ é estimado por máxima verossimilhança concentrada, em que os coeficientes do VAR e a covariância das inovações entram pelos seus estimadores condicionais a $\theta$, e a função objetivo resultante depende apenas de $\theta$. Dividir \ref{eq:5} por $\hat{s}_t$ restaura a homocedasticidade, e o VAR fatorial é estimado por mínimos quadrados ponderados, com peso $1/\hat{s}_t$ em cada linha. A reescala não alcança a extração dos fatores, que recebe o painel na forma original, e nem as cargas $\Lambda$ nem os desvios $s_{iy}$ que multiplicam cada resposta ao impulso são reponderados. A escala é comum às $r$ inovações fatoriais.
...
A base reúne 115 séries mensais da economia brasileira entre janeiro de 2013 e setembro de 2025, totalizando 153 observações alinhadas e 149 inovações fatoriais após as quatro defasagens do VAR. Além dos blocos fiscal e de expectativas já presentes, o painel incorpora o ajuste cambial da DLSP e as medianas Focus para dívida líquida, resultado primário e resultado nominal no ano seguinte. As séries vêm do Banco Central do Brasil, IBGE, B3, Tesouro Nacional e ANBIMA, complementadas por indicadores de risco soberano e mercado externo, como EMBI, CDS de cinco anos e MSCI, e pelo índice de incerteza de política econômica (EPU).
...
A especificação de produção é $(r, q, p) = (5, 5, 4)$. O IC2 de \cite{bai-ng}, na versão BLL padronizada de \cite{barigozzi2016non}, seleciona $r=5$ na amostra completa. Mantemos $q=5$ por decisão explícita; Amengual--Watson BLL condicionado a $r=5,p=4$ seleciona $q=2$, resultado reportado mas não usado para governar a dimensão. O AIC seleciona $p=4$ (8,231267) na amostra comum de 154 observações com constante e tendência; a tendência pertence somente ao critério e o VAR fatorial estimado mantém intercepto.

A máxima verossimilhança de $\theta$ em \ref{eq:5b} devolve $\hat{\theta} = (6{,}611;\ 12{,}468;\ 1{,}760;\ 0{,}944)$, com $t^*$ fixado em março de 2020. O decaimento estimado é lento: a meia-vida de $s_t - 1$ é de doze meses, $s_t$ difere de um em 70 dos 162 meses de inovação, e o total reponderado equivale a 19,1 meses. A janela pré-COVID não é afetada, porque nela $s_t = 1$ em todos os meses.

Os fatores estáticos são extraídos por componentes principais do painel padronizado e sua dinâmica é descrita por um VAR(4), estimado por mínimos quadrados nas linhas divididas por $s_t$. As funções de impulso-resposta pontuais cobrem horizontes de 0 a 36 meses.

\subsection{Relevância do instrumento}

Seguimos \cite{montielolea} para avaliar a relevância do instrumento. A estatística de Wald no espaço dos fatores, $\xi_{mp}$, é $6{,}85$ na amostra completa e $8{,}64$ na janela pré-COVID, enquanto o primeiro estágio robusto, $F^{rob}_{mp}$, vale $11{,}77$ e $13{,}81$, respectivamente. Ambas as estatísticas projetam o instrumento na combinação das inovações fatoriais que determina a normalização no DI de seis meses.
...
A inferência sobre as respostas usa os conjuntos de confiança Anderson--Rubin de \cite{montielolea}, obtidos por inversão de teste em cada série e horizonte, em 68\% e 90\%, com Newey--West de ordem zero. A extensão ao DFM troca apenas os vetores que entram na razão identificada, uma vez que o coordenado que seleciona uma observável dá lugar ao funcional linear estimado $c_j'=s_{jy}\Lambda_j'$, e o da normalização a $c_{mp}'$, enquanto a estatística, a matriz de pesos e a inversão permanecem as dos autores.\footnote{Em \cite{montielolea} a resposta é $e_i'C_h(A)\Gamma/e_1'\Gamma$, com $\Gamma=E[z_t\eta_t]$ e impacto unitário na variável de normalização. No DFM ela é $\delta\,c_i'B_h\Gamma/c_{mp}'\Gamma$, em que $B_h$ é o bloco $r\times r$ superior esquerdo da matriz companheira do VAR fatorial elevada a $h$ e $\delta=0{,}005$ é a normalização no DI de seis meses. Com $q=r$, a projeção das inovações fatoriais sobre o espaço dos choques dinâmicos é a identidade.}

Duas propriedades desses conjuntos precisam ser declaradas. A covariância é do tipo \textit{plug-in}: ela condiciona nas cargas $\Lambda$, nos desvios $s_{iy}$ e na escala $\hat{\theta}$ de \ref{eq:5b}, tratando-os como conhecidos. No caso de $\hat{\theta}$ o argumento usual --- de que o erro de estimação é de ordem menor --- não está disponível, porque cada $\bar{s}$ é o tamanho de um único resíduo e não melhora quando a amostra cresce. Além disso, a covariância exige que a dimensão dos momentos seja menor que o número de inovações, condição que exclui $(r,q)=(8,8)$ e a janela pré-COVID em $p=4$; células excluídas são reportadas como tal, sem substituição por outra inferência.
