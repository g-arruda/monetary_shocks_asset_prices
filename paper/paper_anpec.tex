...
A base reúne 106 séries mensais para a economia brasileira entre janeiro de 2013 e setembro de 2025, o que corresponde a 153 observações alinhadas e 147 inovações fatoriais após as seis defasagens do VAR. O início em 2013 acompanha a mudança metodológica da PNAD Contínua no mercado de trabalho. As fontes são oficiais (Banco Central do Brasil, IBGE, B3, Tesouro Nacional e ANBIMA), somadas a indicadores de risco soberano e mercado externo (EMBI, CDS de cinco anos e MSCI) e a um índice de incerteza de política econômica (EPU). Os blocos cobrem atividade real, mercado de trabalho, inflação, commodities, agregados monetários e de crédito, e preços de ativos.
...
A especificação de produção é $(r, q) = (7, 6)$, fixada na comparação inicial entre quatro candidatos antes da extensão da grade. A \autoref{tab:rq_sweep} usa a grade completa como auditoria posterior e reporta, para o instrumento primário, os dois diagnósticos de relevância propostos por \cite{montielolea}: a Wald $\xi_{mp}$ na direção do impacto sobre o yield de 6 meses e o $F^{rob}_{mp}$ do primeiro estágio na mesma direção. A Wald usa erros de Eicker--White e o instrumento residualizado nas defasagens do VAR dos fatores; o $F^{rob}_{mp}$ é o quadrado da estatística $t$ HC1 do instrumento na regressão da inovação comum do yield de 6 meses sobre o instrumento e os mesmos controles. Valores acima de $10$ sugerem que as bandas convencionais têm cobertura aproximadamente válida, mas os autores desaconselham condicionar a inferência a esse pré-teste.
...
\toprule
 & \multicolumn{2}{c}{Amostra completa} & \multicolumn{2}{c}{Pré-COVID} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
$(r, q)$ & $\xi_{mp}$ & $F^{rob}_{mp}$ & $\xi_{mp}$ & $F^{rob}_{mp}$ \\
\midrule
$(5, 4)$ &  4,77 &  6,85 &  7,89 & 6,96 \\
$(5, 5)$ &  6,34 & 11,12 &  8,04 & 6,08 \\
$(6, 4)$ &  4,30 &  4,52 & 11,13 & 7,01 \\
$(6, 5)$ &  4,76 &  5,13 & 10,94 & 6,83 \\
$(6, 6)$ &  7,78 & 10,30 & 10,44 & 6,61 \\
$(7, 4)$ &  6,49 &  6,25 & 12,20 & 5,48 \\
$(7, 5)$ &  7,58 &  8,00 & 12,05 & 5,36 \\
$(7, 6)$ &  7,65 &  7,95 & 11,53 & 6,26 \\
$(7, 7)$ & 10,92 & 12,75 & 11,55 & 6,19 \\
$(8, 4)$ &  7,13 &  6,65 &  9,57 & 4,76 \\
$(8, 5)$ &  8,56 &  8,88 & 10,50 & 6,39 \\
$(8, 6)$ &  8,55 &  8,86 & 11,08 & 6,69 \\
$(8, 7)$ &  9,02 &  8,96 &  9,83 & 4,18 \\
$(8, 8)$ & 11,62 & 12,44 &  9,51 & 4,10 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item Diagnósticos de relevância de \cite{montielolea} na direção da variável normalizadora. $\xi_{mp}$: Wald com distribuição $\chi^2_1$. $F^{rob}_{mp}$: primeiro estágio HC1 na mesma direção $c_{mp}'\eta_t$. A grade contém todas as combinações com $r \in \{5,\ldots,8\}$ e $q \in \{4,\ldots,r\}$. Instrumento primário, com ortogonalização pré-evento e filtro de sinal. Amostra completa: 2013--2025. Pré-COVID: 2013--2019.
\end{tablenotes}
\end{threeparttable}
\end{table}

A correção do fechamento mensal reduziu a força na amostra completa em toda a vizinhança da especificação congelada. A célula de produção $(7,6)$ tem $\xi_{mp}=7{,}65$ na amostra completa e $11{,}53$ na janela pré-COVID, enquanto $(7,7)$ é a única dimensão em que $\xi_{mp}$ supera 10 nas duas janelas, com 10,92 e 11,55. A produção permanece em $(7,6)$ porque selecionar $(7,7)$ depois de observar a nova grade constituiria reotimização \textit{ex post}. A grade mostra, portanto, que a escolha de dimensão não é o resultado de uma busca pela célula de maior força e que a inferência convencional exige ressalva na amostra completa.
...
Na especificação de produção, $\xi_{mp}$ vale $7{,}65$ na amostra completa e $11{,}53$ na janela pré-COVID; $F^{rob}_{mp}$ vale, respectivamente, $7{,}95$ e $6{,}26$. Os dois diagnósticos ficam abaixo da referência de 10 na amostra completa. Pré-COVID, a Wald cruza 10, mas o $F$ robusto não, em parte porque a correção HC1 é severa com 78 observações efetivas e 42 defasagens fatoriais como controles. A evidência de força nessa janela é, portanto, mista. Como \cite{montielolea} desaconselham selecionar o procedimento inferencial por um pré-teste de força, as bandas do \textit{wild bootstrap} são lidas com cautela nas duas janelas.
...
\end{equation}
em que $\mathbf W_t$ é o vetor-coluna de regressores e $\boldsymbol{\beta}_\ell$ reúne os coeficientes associados à defasagem $\ell$. Duas especificações usam os retornos das séries externas, com $L=3$ e $L=6$\footnote{O bloco externo contém o retorno do S\&P 500 combinado à variação do VIX, o retorno do MSCI de mercados emergentes, a variação do índice de commodities metálicas do Banco Central e a variação do índice de incerteza de política econômica dos Estados Unidos, enquanto o câmbio USD/BRL entra como quinto regressor, de natureza doméstica.}. As outras três usam os sete fatores estimados, com $L=1$, $L=3$ e $L=6$. Em cada especificação, a estatística de Wald robusta a heterocedasticidade testa conjuntamente a hipótese de que todos os vetores de coeficientes são nulos. Como o instrumento vale zero em 94 dos 156 meses do calendário, o valor-$p$ assintótico não é confiável, e os valores reportados vêm de \textit{wild bootstrap} sob a nula com 2.000 réplicas. Nenhuma das cinco especificações rejeita a imprevisibilidade, com $R^2$ entre $0{,}054$ e $0{,}408$ e valores-$p$ de \textit{bootstrap} entre $0{,}199$ e $0{,}946$.

O segundo diagnóstico testa a denominação dos índices de commodities e é distinto das regressões de previsibilidade. Nos dois últimos painéis da \autoref{fig:cambio}, o índice de metais do Banco Central sobe $5{,}67\%$ no impacto e exclui zero a 90\% até $h = 4$, embora uma economia pequena e aberta não mova o preço mundial dos metais. A denominação do índice permite testar se essa resposta vem de um componente global ou do câmbio.

O índice do Banco Central é medido em reais. Para cada índice de commodities $i$, a conversão
\[
C^{USD}_{it} = \frac{C^{BRL}_{it}}{S_t},
\]
em que $S_t$ é a taxa de câmbio em reais por dólar, separa o preço internacional da variação cambial. Em um painel aumentado com os três índices convertidos para dólares, estimado com 200 réplicas, o índice de metais em reais responde $+6{,}30\%$ no impacto e exclui zero em quatro dos cinco primeiros horizontes, enquanto o mesmo índice em dólares responde $+1{,}59\%$ e contém zero nos 25 horizontes testados. Os índices de agropecuária e de energia têm o mesmo padrão, e a correlação entre a variação do índice de metais e a do câmbio é $+0{,}450$ em reais contra $-0{,}409$ em dólares.

Um fator global moveria as duas denominações, mas a versão em dólares contém zero em todos os horizontes. O movimento do índice em reais herda, portanto, parte da resposta cambial, que é $+4{,}48\%$ no mesmo painel aumentado. Como a inclusão das três séries reduz $\xi_{mp}$ para $6{,}14$, a comparação identifica o papel da denominação sem fornecer uma estimativa alternativa da magnitude de produção.

As três séries externas usadas como placebo passam nos dois níveis de leitura adotados na Seção 4. Na banda de 90\%, o primeiro placebo exclui zero em 1 dos 49 horizontes, enquanto o MSCI de mercados emergentes e o índice de incerteza de política econômica dos Estados Unidos não o fazem em nenhum. Na banda de 68\%, o primeiro placebo exclui zero em 3 horizontes e os outros dois em nenhum, somando 3 dos 147 pares.

\begin{figure}[H]
\begin{center}
    \includegraphics[scale=0.7]{fig_placebos.pdf}
\caption{\textbf{Placebos}\\{Séries externas que um choque de política monetária brasileiro não deve mover. O único marcador laranja está em $h = 2$ no primeiro painel. A faixa escura de 68\% exclui zero em 3 dos 147 pares das três séries.}}
\label{fig:placebos}
\end{center}
\end{figure}

Um choque monetário brasileiro não deve mover ativos globais e não os move em nenhuma das duas barras. O que os placebos não descartam é um confundidor doméstico, porque uma surpresa fiscal brasileira também deixaria o S\&P 500 parado, e essa alternativa pode incidir sobre a camada do instrumento.
...
O coeficiente nos 62 dias retidos não é zero, e essa é a primeira ressalva que o resultado carrega. Medido pelo CDS ele vale $0{,}140$ com valor-$p$ de \textit{bootstrap} de $0{,}003$, contra $0{,}099$ e $0{,}109$ do EMBI+, de modo que a afirmação sustentada é a de menos risco soberano que em um dia comum, e não a de ausência de risco. Os dias que o filtro rejeita carregam risco com sinal negativo e significativo ($-0{,}285$, valor-$p$ de $0{,}010$), enquanto o conjunto das 95 reuniões é nulo ($-0{,}004$), o que mostra que a regra separa dois regimes de sinal oposto em vez de descartar ruído.

Ortogonalizar a surpresa ao risco soberano contemporâneo é um limite inferior, porque política monetária pode mover o spread e a operação remove também essa resposta. A estatística $\xi_{mp}$ passa de $7{,}65$ na produção para $7{,}18$ quando a regressão auxiliar usa EMBI+ e câmbio, e para $8{,}97$ quando o CDS entra. Nessa última versão, o câmbio responde $0{,}225$ contra $0{,}228$ na produção e o CDS abre 40,8 contra 43,4 pontos-base, com bandas de 90\% que excluem zero nas duas versões. A força permanece abaixo de 10 na amostra completa, de modo que o exercício preserva a direção das respostas sem eliminar a ressalva de instrumento fraco.
...
A distinção entre valores e seleção mostra onde o bloco global afeta o instrumento. Retirar esse bloco apenas dos valores reduz $\xi_{mp}$ de $7{,}65$ para $7{,}57$, enquanto rederivar também a máscara reduz a estatística para $6{,}59$. A diferença decorre de o bloco explicar $0{,}76\%$ da variância da surpresa de juros e $24{,}7\%$ da variância do retorno acionário. A bateria não detecta confundimento no bloco contemporâneo observado, mas não exclui spillovers que cheguem por variáveis fora desse bloco.
